//g++ -std=c++17 -Wall -Wextra maxmin2.cpp -o maxmin2 でビルドする
...
int search_min(int x,int y){ //一回食べて、相手の出方を全通り試してその最小値を返す(最小値が最善手)
    ...
    for(int i=0;i<y+1;i++){ //1回目の食べる(CPU)
        for(int j=x;j<W;j++){
            Im_board1[i][j]=0;
        }
    }
    for(int i=0;i<H;i++){
        for(int j=0;j<W;j++){
            if(Im_board1[i][j]==1){ //2回目の食べるところ(人間の手を全通り試す)
                 vector<vector<int>> b(H,vector<int>(W));;
                    int ans =0;
                    for(int i=0;i<H;i++){ //コピー
                        for(int j=0;j<W;j++){
...
void str() { //max-min法を用いる また、毒のマスを2にしている。全探索で効いてくる
    ...
    if(cnt_0==0){
        ...
        for(int i=0;i<H;i++){ //最大値求めた
            for(int j=0;j<W;j++){
                ans =max(0,check_max[i][j]);
            }
        }
        int x,y;
        for(int i=0;i<H;i++){ //最大値の場所探した
            for(int j=0;j<W;j++){
                if(ans==check_max[i][j]){
                    x=j;
                    y=i;
                    break;
                }
